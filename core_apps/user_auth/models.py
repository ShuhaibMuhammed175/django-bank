...
class User(AbstractUser):
    class SecurityQuestions(models.TextChoices):
        ...
        FAVORITE_COLOR = (
            "favorite_color",
            _("What is your mother's favorite color?"),
        )
        BIRTH_CITY = (
            "birth_city",
            _("What is the name of the city where you born?"),
        )
        CHILDHOOD_FRIENDS = (
            "child_friends",
            _("What is the name of your childhood friends?"),
        )
    ...
    last_failed_login = models.DateTimeField(_("Last Login Attempt"), blank=True, null=True)
    otp = models.CharField(_("OTP"), max_length=6, blank=True)
    otp_expiry_time = models.DateTimeField(_("OTP Expiry"), blank=True, null=True)

    objects = UserManager()
    USERNAME_FIELD = "email"
    REQUIRED_FIELDS = ["first_name", "last_name", "id_no", "security_questions", "security_answer"]
    ...
    def full_name(self) -> str:
        ful_name = f"{self.first_name} {self.last_name}"
        return ful_name.title().strip()
