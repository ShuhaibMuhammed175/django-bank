...
def create_bank_account(user, currency: str, account_type: str) -> str:
    with transaction.atomic():
        ...
        """
        ️⃣ BankAccount.objects.filter(user=user)
        → Search in the database for bank accounts that belong to this user.
        
        2️⃣ .exists()
        → This asks: "Did we find any?"
        It returns:
        
        True if at least one bank account already exists for this user
        
        False if the user has no bank account yet
