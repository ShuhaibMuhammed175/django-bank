...
class DepositSerializer(serializers.ModelSerializer):
    ...
    """
    validate_account_number is a method related to account_number fields.
    validate_<fieldname>() is directly tied to the field name. 
    """
    ...
    """
    to_representation() is a built-in serializer method
    You override it only when you want to change the output format
    Its purpose is to control what the API returns to the user.
    
    So simply saying, it allows us to customize the outgoing API data by changing the default behavior of how DRF serializes the model into JSON.
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    """
    Why to_representation() is used here?

    Because the raw database values are not user-friendly, 
    and the API wants to return readable information to the frontend.
    
    Without overriding to_representation(), DRF would return something like:
